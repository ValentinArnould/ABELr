"""
How many SEEDS (manually edited photos) are needed per catalog to
calibrate the WB Temp ~ as-shot model and the exposure?

For k randomly drawn seeds: fit Temp ~ a*r/g + b*b/g + c on k photos,
predict the rest -> RMSE. Tint and Exposure: take the median of the seeds.
Also compare 'fixed physical slope (a=2450) + intercept from seeds'.
...
SLOPE_PRIOR = 2450.0  # K per r/g unit, stable ILCE-7M4 (measured 2436/2459/2464)
...
def main():
    try:
        ...
        pass
    for name, data in [("CGC (cold, n=%d)", load_cgc()), ("YGGDRASIL (artistic, n=%d)", load_yggdrasil())]:
        print("\n" + "=" * 64)
        print(name % len(data))
