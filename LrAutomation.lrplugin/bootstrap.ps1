...
# cassee. $PSScriptRoot = LrAutomation.lrplugin/ (le plugin embarque tout : app/,
...
Write-Host '[Lr Automation] Bootstrap -- verification de Python...'
...
if ($verOutput -match 'Python (\d+)\.(\d+)') {
    ...
    if ($maj -lt 3 -or ($maj -eq 3 -and $min -lt 11)) {
        Write-Error "Python $maj.$min detecte -- 3.11+ requis pour Lr Automation."
        exit 1
    }
    Write-Host "[Lr Automation] Python $maj.$min OK."
} else {
    Write-Warning "Version Python non reconnue (`"$verOutput`") -- poursuite quand meme."
}

if (-not (Test-Path $venvPy)) {
    Write-Host "[Lr Automation] Creation du venv : $venvDir"
    ...
    Write-Host '[Lr Automation] venv deja present -- reutilise.'
}

Write-Host '[Lr Automation] Mise a jour de pip...'
...
if ($hasNvidia) {
    Write-Host '[Lr Automation] GPU NVIDIA detecte -- installation torch CUDA (cu124)...'
    & $venvPy -m pip install torch==2.6.0 torchvision==0.21.0 --index-url https://download.pytorch.org/whl/cu124
} else {
    Write-Host '[Lr Automation] Aucun GPU NVIDIA detecte -- installation torch CPU (fallback).'
...
Write-Host '[Lr Automation] Installation des autres dependances (requirements.txt)...'
...
Write-Host '[Lr Automation] Bootstrap termine.'
