...
function Metadata.setRating(photoIds, rating)
    ...
    local applied = applyBatched('Lr Automation : note', matched, errors, function(photo)
...
function Metadata.setFlagColor(photoIds, flag, color)
    ...
    local applied = applyBatched('Lr Automation : flag/label', matched, errors, function(photo)
...
function Metadata.setKeywords(photoIds, addNames, removeNames)
    ...
    local addKw = {}
    if #addNames > 0 then
        catalog:withWriteAccessDo('Lr Automation : mots-clés', function()
    ...
    local applied = applyBatched('Lr Automation : mots-clés', matched, errors, function(photo)
