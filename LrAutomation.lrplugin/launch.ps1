...
# $PSScriptRoot = LrAutomation.lrplugin/ (le plugin est auto-suffisant : embarque
...
if (-not (Test-Path $venvPy)) {
    Write-Host '[Lr Automation] Premiere installation detectee -- lancement du bootstrap (venv + dependances)...'
    & (Join-Path $root 'bootstrap.ps1')
    if (-not (Test-Path $venvPy)) {
        Write-Host ''
        Write-Host '[Lr Automation] ERREUR : bootstrap termine mais python.exe introuvable dans le venv.' -ForegroundColor Red
...
if ($exitCode -ne 0) {
    Write-Host ''
    Write-Host "[Lr Automation] L'application s'est arretee avec le code $exitCode." -ForegroundColor Red
    Read-Host 'Appuyez sur Entree pour fermer cette fenetre'
}
exit $exitCode
